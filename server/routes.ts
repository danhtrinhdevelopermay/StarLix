...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/video-status/:taskId", async (req, res) => {
    try {
      ...
      let generation;
      try {
        generation = await storageInstance.getVideoGenerationByTaskId(taskId);
      } catch (dbError) {
  ...
  app.get("/api/generations", requireAuth, async (req: Request & { user?: User }, res: Response) => {
    try {
      ...
      const storageInstance = await storage();
      let generations = await storageInstance.getUserVideoGenerations(req.user.id);
      ...
      if (hasUpdates) {
        generations = await storageInstance.getUserVideoGenerations(req.user.id);
      }
