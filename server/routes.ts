...
async function authenticateExternalApiKey(req: Request, res: Response, next: NextFunction) {
  ...
  try {
    // Using mysqlStorage directly
    const externalApiKey = await mysqlStorage.getExternalApiKeyByKey(apiKey);
...
async function getBestApiKey(): Promise<{ key: string; apiKeyId?: string } | null> {
  try {
    ...
    // Using mysqlStorage directly
    const activeKeys = await mysqlStorage.getActiveApiKeys();
    ...
    for (const dbApiKey of activeKeys) {
      ...
      if (cached && (Date.now() - cached.lastChecked) < 1000) { // Recently updated
        await mysqlStorage.updateApiKey(dbApiKey.id, {
          credits,
...
async function enhanceVideo(generationId: string, videoUrl: string, storageInstance: any) {
  try {
    ...
    await mysqlStorage.updateVideoGeneration(generationId, {
      status: "completed",
...
async function startAutoRefresh() {
  ...
  const refreshCredits = async () => {
    try {
      ...
      // Using mysqlStorage directly
      const allApiKeys = await mysqlStorage.getAllApiKeys();
      
      for (const apiKey of allApiKeys) {
        try {
          const credits = await checkApiCredits(apiKey.apiKey);
          await mysqlStorage.updateApiKey(apiKey.id, {
            credits,
            lastChecked: new Date(),
...
export async function addUserToRequest(req: Request & { user?: User }, res: Response, next: NextFunction) {
  if (req.session?.userId) {
    try {
      const user = await mysqlStorage.getUserById(req.session.userId);
      if (user) {
        req.user = user;
      }
    } catch (error) {
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // SEO and Static File Routes - Must be first to ensure proper serving
  ...
  app.get("/og-image.jpg", (_req, res) => {
    res.type("image/jpeg");
    res.sendFile(path.join(process.cwd(), "client/public/og-image.jpg"));
  });
  ...
  app.post("/api/check-device", async (req, res) => {
    try {
      ...
      // Simple device check for MySQL
      const existingUser = await mysqlStorage.getUserByDeviceId(deviceId);
      const result = existingUser 
        ? { canRegister: false, reason: "Thiết bị này đã được sử dụng để đăng ký tài khoản" }
        : { canRegister: true };
  ...
  app.post("/api/register", async (req, res) => {
    try {
      ...
      const existingUser = await mysqlStorage.getUserByUsername(validatedData.username);
      if (existingUser) {
      ...
      if (validatedData.deviceId) {
        const deviceUser = await mysqlStorage.getUserByDeviceId(validatedData.deviceId);
        if (deviceUser) {
          return res.status(409).json({ message: "Thiết bị này đã được sử dụng để đăng ký tài khoản" });
        }
      }
      
      const user = await mysqlStorage.createUser(validatedData);
  ...
  app.post("/api/login", async (req, res) => {
    try {
      ...
      const user = await mysqlStorage.validateUserPassword(username, password);
  ...
  app.post("/api/get-credit", requireAuth, async (req: Request & { user?: User }, res: Response) => {
    try {
      ...
      const result = await mysqlStorage.createRewardClaim(req.user.id);
  ...
  app.get("/api/reward-claims", requireAuth, async (req: Request & { user?: User }, res: Response) => {
    try {
      ...
      // Using mysqlStorage directly
      const rewardClaims = await mysqlStorage.getRewardClaims(req.user.id);
  ...
  app.get("/api/daily-link-usage", requireAuth, async (req: Request & { user?: User }, res: Response) => {
    try {
      ...
      // Using mysqlStorage directly
      const stats = await mysqlStorage.getDailyLinkUsageStats();
  ...
  app.get("/api/claim-reward/:token", async (req: Request, res: Response) => {
    try {
      ...
      const result = await mysqlStorage.claimReward(token);
  ...
  app.post("/api/generate-video", requireAuth, async (req: Request & { user?: User }, res: Response) => {
    try {
      ...
      const generation = await mysqlStorage.createVideoGeneration({
        ...validatedData,
        userId: user.id,
      }, totalCredits);
      ...
      if (data.code !== 200) {
        await mysqlStorage.updateVideoGeneration(generation.id, {
          status: "failed",
  ...
  app.post("/api/object-replacement", requireAuth, async (req: Request & { user?: User }, res: Response) => {
    try {
      ...
      const replacement = await mysqlStorage.createObjectReplacement({
        prompt: validatedData.prompt,
      ...
      await mysqlStorage.updateUserCredits(user.id, user.credits - totalCredits);

      // Get available PhotAI API keys
      const photaiKeys = await mysqlStorage.getPhotAIApiKeys();
      ...
      if (photaiKeys.length === 0) {
        ...
        if (!response.ok || data.status_code !== 200 || data.message !== "success") {
          await mysqlStorage.updateObjectReplacement(replacement.id, {
            status: "failed",
            errorMessage: data.error || data.message || 'Object replacement failed',
          });
          // Refund credits
          await mysqlStorage.updateUserCredits(user.id, user.credits);
      ...
      for (const apiKeyRecord of photaiKeys) {
        ...
        try {
          ...
          if (response.ok && data.status_code === 200 && data.message === "success") {
            ...
            await mysqlStorage.incrementExternalApiKeyUsage(apiKeyRecord.id, 1);
  ...
  app.get("/api/object-replacements", requireAuth, async (req: Request & { user?: User }, res: Response) => {
    try {
      ...
      // Using mysqlStorage directly
      let replacements = await mysqlStorage.getUserObjectReplacements(req.user.id);
      ...
      replacements = await mysqlStorage.getUserObjectReplacements(req.user.id);
  ...
  app.post("/api/photai-tools", requireAuth, async (req: Request & { user?: User }, res: Response) => {
    try {
      ...
      const operation = await mysqlStorage.createPhotaiOperation({
      ...
      if (photaiKeys.length === 0) {
        ...
        if (response.ok && data.status_code === 200 && data.message === "success") {
          ...
          if (data.output_image) {
            const resultImageUrl = data.output_image;
            await mysqlStorage.updatePhotaiOperation(operation.id, {
              status: "completed",
              resultImageUrl,
              completedAt: new Date(),
            });
  ...
  app.get("/api/photai-operations", requireAuth, async (req: Request & { user?: User }, res: Response) => {
    try {
      ...
      // Using mysqlStorage directly
      let operations = await mysqlStorage.getUserPhotaiOperations(req.user.id);
      ...
      operations = await mysqlStorage.getUserPhotaiOperations(req.user.id);
  ...
  app.get("/api/video-status/:taskId", async (req, res) => {
    try {
      ...
      console.log(`🔄 Starting video status check for taskId: ${taskId}`);
      ...
      let generation;
      try {
        generation = await mysqlStorage.getVideoGenerationByTaskId(taskId);
      } catch (dbError) {
        console.error(`❌ Database error when fetching generation ${taskId}:`, dbError);
        return res.status(500).json({ message: "Database error" });
      }
      
      // Check if video was just created (within 30 seconds) - give VEO3 time to process
      if (generation && generation.createdAt) {
        const createdAt = new Date(generation.createdAt);
        const ageInSeconds = (new Date().getTime() - createdAt.getTime()) / 1000;
        
        // Check for timeout first (8 minutes = 480 seconds)
        if (ageInSeconds > 480 && generation.status === "processing") {
          console.log(`⏰ Video ${taskId} has been processing for ${Math.round(ageInSeconds)}s (>${Math.round(480)}s timeout) - marking as failed`);
          
          await mysqlStorage.updateVideoGeneration(generation.id, {
            status: "failed",
            errorMessage: "Video đã vượt quá thời gian xử lý bình thường (8 phút). Có thể STLIX đang quá tải hoặc có lỗi xảy ra.",
            completedAt: new Date(),
          });
        ...
        if (ageInSeconds < 30) {
          console.log(`⏳ Video ${taskId} was created ${Math.round(ageInSeconds)}s ago - still too early to check, returning processing status`);
          return res.json({
            successFlag: 0,
            status: "processing",
      ...
      const response = await fetch(`${VEO3_API_BASE}/query/${taskId}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${apiKeyData.key}`,
        },
      ...
      if (!response.ok) {
        ...
        console.log(`❌ VEO3 API Error ${response.status} for ${taskId}:`, errorData);
        
        if (generation) {
          let userMessage = "Có lỗi xảy ra khi tạo video";
          let technicalMessage = errorData.msg || errorData.message || 'Failed to check status';
          
          // Handle specific error cases with user-friendly Vietnamese messages
          if (response.status === 400) {
            userMessage = "Nội dung video không hợp lệ hoặc vi phạm chính sách của VEO3";
            technicalMessage = "Bad request - content may violate policies";
          } else if (response.status === 403) {
            userMessage = "Video bị từ chối do vi phạm chính sách hoặc quyền hạn không đủ";
            technicalMessage = "Forbidden - policy violation or insufficient permissions";
          } else if (response.status === 404) {
            // For 404, check if video is very new - might just need more time
            if (generation.createdAt) {
              const createdAt = new Date(generation.createdAt);
              const ageInMinutes = (new Date().getTime() - createdAt.getTime()) / (1000 * 60);
              
              if (ageInMinutes < 3) {
                console.log(`⏳ Video ${taskId} is only ${Math.round(ageInMinutes * 60)}s old and got 404 - might still be processing, returning processing status`);
                return res.json({
                  successFlag: 0,
                  status: "processing", 
                  message: "Video đang được khởi tạo trên hệ thống VEO3. Vui lòng đợi thêm..."
                });
              }
            }
            
            userMessage = "Video không tìm thấy trên hệ thống VEO3. Có thể do:\n• Video đang được xử lý và chưa sẵn sàng\n• Prompt vi phạm chính sách và bị từ chối\n• Lỗi hệ thống VEO3\n\nVui lòng thử tạo video mới với prompt khác.";
            technicalMessage = "Video not found on VEO3 API (may be processing, rejected, or expired)";
          } else if (response.status === 410) {
            userMessage = "Video đã bị xóa vĩnh viễn khỏi hệ thống VEO3";
            technicalMessage = "Video permanently deleted from VEO3 system";
          } else if (response.status === 429) {
            userMessage = "Quá nhiều yêu cầu. Vui lòng thử lại sau ít phút";
            technicalMessage = "Rate limit exceeded";
          } else if (response.status >= 500) {
            userMessage = "Lỗi máy chủ VEO3. Vui lòng thử lại sau";
            technicalMessage = "VEO3 server error";
          }
          
          // Check error message content for policy violations
          const errorMsg = (errorData.msg || errorData.message || '').toLowerCase();
          if (errorMsg.includes('policy') || errorMsg.includes('violation') || 
              errorMsg.includes('inappropriate') || errorMsg.includes('banned') || 
              errorMsg.includes('restricted') || errorMsg.includes('not allowed')) {
            userMessage = "Video vi phạm chính sách nội dung của VEO3 và đã bị từ chối";
            technicalMessage = `Policy violation: ${errorData.msg || errorData.message}`;
          } else if (errorMsg.includes('deleted') || errorMsg.includes('removed')) {
            userMessage = "Video đã bị xóa khỏi hệ thống VEO3";
            technicalMessage = `Video removed: ${errorData.msg || errorData.message}`;
          } else if (errorMsg.includes('expired') || errorMsg.includes('not found')) {
            userMessage = "Video không tìm thấy hoặc đã hết hạn trên hệ thống VEO3";
            technicalMessage = `Video expired: ${errorData.msg || errorData.message}`;
          }
        ...
        throw new Error(errorData.msg || errorData.message || 'Failed to check status');
      }
      ...
      if (!data.data || data.data === null || !data.data.status) {
        console.log(`⏳ Video ${taskId} still processing - VEO3 returned null/incomplete data`);
        
        // Check if video is very old (more than 8 minutes) and should be marked as failed
        if (generation && generation.createdAt) {
          const createdAt = new Date(generation.createdAt);
          const ageInMinutes = (new Date().getTime() - createdAt.getTime()) / (1000 * 60);
          
          if (ageInMinutes > 8) {
            console.log(`⏰ Video ${taskId} is ${Math.round(ageInMinutes)} minutes old - marking as failed due to timeout`);
            await mysqlStorage.updateVideoGeneration(generation.id, {
              status: "failed",
              errorMessage: "Video generation timed out - VEO3 API returned no data after 8 minutes",
            });
            
            return res.json({
              successFlag: -1,
              status: "failed",
              errorMessage: "Tạo video quá lâu (>8 phút). Có thể do:\n• Prompt quá phức tạp\n• Hệ thống VEO3 đang quá tải\n• Nội dung vi phạm chính sách\n\nVui lòng thử lại với prompt đơn giản hơn."
            });
          }
        }
      ...
      console.log(`✅ Video ${taskId} status check - status: ${data.data.status}`);
      ...
      if (generation) {
        ...
        if (data.data.status === "completed" && data.data.output?.url) {
          ...
          completedAt = new Date();
          console.log(`🎬 Video ${taskId} completed with URL: ${data.data.output.url}`);
          ...
          if (generation.model === "veo3" && generation.enhancementStatus === "none" && resultUrls.length > 0) {
            ...
            enhanceVideo(generation.id, resultUrls[0], mysqlStorage);
          } else {
          ...
          // Return the updated response with correct structure for frontend
          res.json({
            successFlag: 1,
            status,
            response: {
  ...
  app.get("/api/get-1080p/:taskId", async (req, res) => {
    try {
      ...
      // Using mysqlStorage directly
      const generation = await mysqlStorage.getVideoGenerationByTaskId(taskId);
      if (generation) {
        await mysqlStorage.updateVideoGeneration(generation.id, {
  ...
  app.get("/api/generations", requireAuth, async (req: Request & { user?: User }, res: Response) => {
    try {
      ...
      let generations = await mysqlStorage.getUserVideoGenerations(req.user.id);
      
      // Auto-cleanup stuck videos (processing for more than 8 minutes)
      let hasUpdates = false;
      for (const generation of generations) {
        if (generation.status === "processing" && generation.createdAt) {
          const createdAt = new Date(generation.createdAt);
          const ageInSeconds = (new Date().getTime() - createdAt.getTime()) / 1000;
          
          if (ageInSeconds > 480) { // 8 minutes timeout
            console.log(`⏰ Auto-cleanup: Video ${generation.taskId} has been processing for ${Math.round(ageInSeconds)}s - marking as failed`);
      ...
      // If we updated any generations, fetch fresh data
      if (hasUpdates) {
        generations = await mysqlStorage.getUserVideoGenerations(req.user.id);
      }
  ...
  app.get("/api/admin/settings", async (req, res) => {
    try {
      const settings = await mysqlStorage.getAllSettings();
      res.json(settings);
    } catch (error) {
  ...
  app.post("/api/admin/settings", async (req, res) => {
    try {
      ...
      // Using mysqlStorage directly
      const setting = await mysqlStorage.setSetting(key, value);
      res.json(setting);
    } catch (error) {
  ...
  app.get("/api/model-status/veo3-premium", async (req, res) => {
    try {
      // Using mysqlStorage directly
      const setting = await mysqlStorage.getSetting("VEO3_PREMIUM_ENABLED");
  ...
  app.get("/api/admin/api-keys", async (req, res) => {
    try {
      // Using mysqlStorage directly
      const apiKeys = await mysqlStorage.getAllApiKeys();
      res.json(apiKeys);
    } catch (error) {
  ...
  app.post("/api/admin/api-keys", async (req, res) => {
    try {
      ...
      const apiKey = await mysqlStorage.createApiKey(apiKeyData);
      ...
      const updatedApiKey = await mysqlStorage.updateApiKey(apiKey.id, {
        credits,
        lastChecked: new Date(),
        isActive: credits > 0
      });
  ...
  app.put("/api/admin/api-keys/:id", async (req, res) => {
    try {
      ...
      const apiKey = await mysqlStorage.updateApiKey(id, updates);
  ...
  app.delete("/api/admin/api-keys/:id", async (req, res) => {
    try {
      // Using mysqlStorage directly
      const { id } = req.params;
      const success = await mysqlStorage.deleteApiKey(id);
  ...
  app.put("/api/admin/api-keys/:id/toggle", async (req, res) => {
    try {
      ...
      const apiKey = await mysqlStorage.updateApiKey(id, { isActive });
  ...
  app.get("/api/admin/external-api-keys", async (req, res) => {
    try {
      // Using mysqlStorage directly
      const externalApiKeys = await mysqlStorage.getUserExternalApiKeys('admin'); // Get all for admin view
  ...
  app.post("/api/admin/external-api-keys", async (req, res) => {
    try {
      ...
      // Using mysqlStorage directly
      const externalApiKey = await mysqlStorage.createExternalApiKey({
        keyName,
  ...
  app.patch("/api/admin/external-api-keys/:id", async (req, res) => {
    try {
      ...
      // Using mysqlStorage directly
      const updatedKey = await mysqlStorage.updateExternalApiKey(id, updates);
  ...
  app.post("/api/admin/photai-api-keys", async (req, res) => {
    try {
      ...
      const photaiApiKey = await mysqlStorage.createExternalApiKey({
      ...
      await mysqlStorage.updateExternalApiKey(photaiApiKey.id, {
        apiKey: apiKey
      });
  ...
  app.delete("/api/admin/photai-api-keys/:id", async (req, res) => {
    try {
      ...
      // Using mysqlStorage directly
      const success = await mysqlStorage.deleteExternalApiKey(id);
  ...
  app.post("/api/admin/external-api-keys/:id/reset-usage", async (req, res) => {
    try {
      ...
      // Using mysqlStorage directly
      const updatedKey = await mysqlStorage.resetMonthlyUsage(id);
  ...
  app.post("/api/external/generate/text-to-video", authenticateExternalApiKey, async (req: Request & { externalApiKey?: ExternalApiKey }, res: Response) => {
    try {
      ...
      await mysqlStorage.incrementExternalApiKeyUsage(externalApiKey.id, creditsNeeded);
