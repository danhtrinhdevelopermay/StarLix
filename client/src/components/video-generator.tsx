...
- Chỉ trả về prompt đã được mở rộng, không thêm giải thích`;
  ...
  useEffect(() => {
    // Always check for timeout first, regardless of video status
    if (loadingStartTime && isLoadingModalOpen) {
      const elapsedSeconds = (new Date().getTime() - loadingStartTime.getTime()) / 1000;
      const timeoutSeconds = 480; // 8 minutes for veo3_fast
      
      if (elapsedSeconds > timeoutSeconds) {
        setIsLoadingModalOpen(false);
        setCurrentTaskId(""); // Clear the task ID to stop polling
        showPopup(
          "Timeout - Video quá lâu", 
          "Video đã vượt quá thời gian xử lý bình thường (8 phút). Có thể STLIX đang quá tải. Vui lòng thử tạo video mới với prompt ngắn gọn hơn.", 
          "warning"
        );
        return; // Exit early if timed out
      }
    }
    ...
    if (videoStatus?.successFlag === 1 && videoStatus?.response?.resultUrls?.[0]) {
      ...
      setCurrentTaskId(""); // Clear task ID to stop polling
      toast({
    ...
    // Handle processing status with progress notifications
    else if (videoStatus?.status === "processing" && loadingStartTime) {
      const elapsedSeconds = (new Date().getTime() - loadingStartTime.getTime()) / 1000;
      
      if (elapsedSeconds > 180) { // 3 minutes
        const timeoutSeconds = 480;
  ...
  }, [videoStatus, toast, loadingStartTime, currentTaskId, isLoadingModalOpen]);
  ...
  const generateVideoMutation = useMutation({
    ...
    onError: (error) => {
      ...
      if (error instanceof Error) {
        ...
        // Check for policy violations and content issues first (most important)
        if (errorMessage.includes('policy') || errorMessage.includes('violation') || 
            errorMessage.includes('vi phạm chính sách') || errorMessage.includes('bị từ chối')) {
          title = "Nội dung vi phạm chính sách";
          description = "Nội dung video của bạn vi phạm chính sách của VEO3 và đã bị từ chối. Vui lòng kiểm tra lại nội dung và thử lại.";
        }
        // Check for content removal or deletion
        else if (errorMessage.includes('removed') || errorMessage.includes('deleted') || 
                 errorMessage.includes('bị xóa') || errorMessage.includes('đã bị xóa')) {
          title = "Video bị xóa";
          description = "Video đã bị xóa khỏi hệ thống VEO3. Có thể do vi phạm chính sách hoặc lỗi hệ thống.";
        }
        // Check for inappropriate content
        else if (errorMessage.includes('inappropriate') || errorMessage.includes('banned') || 
                 errorMessage.includes('restricted') || errorMessage.includes('not allowed')) {
          title = "Nội dung không phù hợp";
          description = "Nội dung video không được phép hoặc bị hạn chế trên hệ thống VEO3.";
        }
        // Check for server overload/500 errors
        else if (errorMessage.includes('500') || errorMessage.includes('server') || errorMessage.includes('overload')) {
          title = "Máy chủ quá tải";
          description = "Máy chủ đang xử lý quá nhiều yêu cầu. Vui lòng thử lại sau vài phút.";
        }
        ...
        // Check for bad request (content validation errors)
        else if (errorMessage.includes('400') || errorMessage.includes('bad request') || 
                 errorMessage.includes('invalid') || errorMessage.includes('không hợp lệ')) {
          title = "Nội dung không hợp lệ";
          description = "Nội dung hoặc thông số video không hợp lệ. Vui lòng kiểm tra lại và thử lại.";
        }
        // Other errors
        else {
