...
export default function VideoPreview({ videoUrl, taskId, onVideoLoad }: VideoPreviewProps) {
  ...
  useEffect(() => {
    ...
    if (startTime && pollingTaskId) {
      const elapsedSeconds = (new Date().getTime() - startTime.getTime()) / 1000;
      const timeoutSeconds = 480; // 8 minutes timeout
      
      if (elapsedSeconds > timeoutSeconds) {
        console.log(`⏰ Video ${pollingTaskId} timed out after ${Math.round(elapsedSeconds)}s`);
        setProgress(0);
    ...
    if (videoStatus?.successFlag === 1) {
      ...
      if (resultUrls && resultUrls.length > 0) {
        ...
        setPollingTaskId(""); // Clear polling task ID to stop polling
        onVideoLoad(url);
        toast({
      ...
      console.log(`❌ Video failed for ${pollingTaskId}:`, videoStatus.errorMessage);
      setProgress(0);
      setPollingTaskId(""); // Stop polling immediately when failed
      
      // Handle specific error messages from server
      const errorMessage = videoStatus?.errorMessage || "Có lỗi xảy ra khi tạo video";
      let title = "Tạo video thất bại";
      let type: "error" | "warning" = "error";
      
      // Check for policy violation or content issues
      if (errorMessage.includes("vi phạm chính sách") || errorMessage.includes("bị từ chối")) {
        title = "Nội dung vi phạm chính sách";
        type = "warning";
      } else if (errorMessage.includes("bị xóa") || errorMessage.includes("đã bị xóa")) {
        title = "Video đã bị xóa";
        type = "warning";
      } else if (errorMessage.includes("không tìm thấy") || errorMessage.includes("hết hạn")) {
        title = "Video không tìm thấy";
        type = "warning";
      } else if (errorMessage.includes("quá lâu") || errorMessage.includes("timeout")) {
        title = "Tạo video quá lâu";
        type = "warning";
      ...
      showPopup(title, errorMessage, type);
      
      // Also show toast for immediate notification
      toast({
        title: title,
  ...
  }, [videoStatus, onVideoLoad, toast, pollingTaskId, startTime]);
